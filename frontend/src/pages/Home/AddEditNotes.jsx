...
const addNewNote=async()=>{
 ...
 if(response.data && response.data.note){
    showToastMessage("Note Added Successdully")
    getAllNotes()
    onClose()
 }
 }catch(error){
...
const editNote=async()=>{
 ...
 if(response.data && response.data.note){
    showToastMessage("Note Updated Successdully")
    getAllNotes()
    onClose()
 }
 }catch(error){
    ...
    <div className='relative'>
      <button className='w-10 h-10 rounded-full flex items-center justify-center absolute -top-3 -right-3 hover:bg-slate-600' onClick={onClose}>
        ...
        <div className='flex flex-col gap-2'>
            ...
            className='text-2xl text-slate-950 outline-none' placeholder='Go To Gym At 5am' />
        </div>
